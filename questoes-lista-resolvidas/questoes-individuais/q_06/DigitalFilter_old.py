...
class DigitalFilter :
    ...
    def filter_params(self, omega_p, omega_s, alpha_p, alpha_s, type="lowpass", Ts=None, warping=False):
        parameters = []
    ...
    # ✅
    # ============================================================
    # Função para calcular a ordem e a frequência de corte do Butterworth
    def order_butter(self, parameter, oc_type="mean"):
        ...
        # Cálculo da Ordem Mínima do Filtro:
        K = np.log10((10**(alpha_s/10) - 1) / (10**(alpha_p/10) - 1)) / (2 * np.log10(Omega_s / Omega_p))
        K = np.ceil(K)
    ...
    def prototype_lp_cheby(self, order, epsilon):
        ...
        polos = -Omega_p * np.sin(np.pi * (2*i-1) / (2*order)) * np.sinh(np.arcsinh(1/epsilon)/order) + 1j*Omega_p * np.cos(np.pi*(2*i-1)/(2*order)) * np.cosh(np.arcsinh(1/epsilon)/order)
    ...
    def low_pass(self, order, Omega_c, epsilon=None, filter_type='butter'):
        ...
        ======================
            num : array
                The numerator coefficients of the prototype filter polynomial
            den : array   
                The denominator coefficients of the prototype filter polynomial
            Omega_c : float
                The cutoff frequency of the desired low-pass filter (rad/s)
        ======================
        Returns:
        ======================
            new_num : array
    ...
    def high_pass(self, order, Omega_c, epsilon=None, filter_type='butter'):
    ...
    def band_pass(self, order, Omega_p, Omega_c=None, epsilon=None, filter_type='butter'):
        ...
        if filter_type == 'butter' and Omega_c != None:
            att_num, att_den, gain_n = self.low_pass(order, Omega_c, filter_type=filter_type)
        ...
        # novo numerador:
        new_num = new_ganho * np.real(np.prod(-polos)) * ((Omega_p[1] - Omega_p[0])**N) * np.real(np.poly(new_zeros))
    ...
    def bilinear_transform(self, num, den, Ts):
        ...
        Kn = K * np.real(np.prod(1 - zeros) / np.prod(1 - polos))
    ...
    def butterworth_filter(self, omega_p, omega_s, alpha_p, alpha_s, type_response="lowpass", oc_type="mean", Ts=None, warping=False):
            '''
            ...
            parametros_prot = self.filter_params(omega_p, omega_s, alpha_p, alpha_s, type=type_response, Ts=Ts, warping=warping)
            ...
            if type_response == "lowpass":
                ...
                num, den = self.high_pass(K, Omega_c, filter_type=filter_type)
    ...
    def chebyshev_filter(self, omega_p, omega_s, alpha_p, alpha_s, type_response="lowpass", Ts=None, warping=False):
            '''
            ...
            if type_response == "lowpass":
                num, den, _= self.low_pass(K, omega_p, epsilon=epsilon, filter_type=filter_type)
            
            elif type_response == "highpass":
                num, den = self.high_pass(K, omega_p, epsilon=epsilon, filter_type=filter_type)
            
            elif type_response == "bandpass":
                num, den = self.band_pass(K, omega_p, epsilon=epsilon, filter_type=filter_type)
           
            elif type_response == "bandstop":
                num, den = self.band_stop(K, omega_p, epsilon=epsilon, filter_type=filter_type)
    ...
    def plot_response(self, num, den, Ts, params=[]):
        ...
        if Ts == None:
            w = np.arange(0, params[1]*1.2, Ts)
            H = np.polyval(num, (1j*w))/np.polyval(den, (1j*w))
        
        else:
            w = np.arange(0, np.pi, Ts)
        ...
        plt.grid()
        # plt.ylim(-60, 5)
        # plt.axhline(-alpha_p, color='red', label='alpha_p', linestyle='--')  # Linha de -3 dB
        # plt.axvline(Omega_p[0], color='blue', linestyle='--')  # Linha de -3 dB
        # plt.axvline(Omega_s[0], color='green', linestyle='--')  # Linha de -3 dB
        # plt.axhline(-alpha_s, color='black', linestyle='--')  # Linha de -3 dB
        # plt.legend()
...
# chebyshev
b , a = signal.cheby1(ordem, ripple, fc_norm, btype='lowpass')  # cálculo dos coeficientes do filtro
'''
...
# num, den = df.chebyshev_filter(Omega_p, Omega_s, alpha_p, alpha_s, type_response="highpass", Ts=Ts, warping=True)
# num, den = df.butterworth_filter(Omega_p, Omega_s, alpha_p, alpha_s, type_response="highpass", Ts=Ts, warping=True)

# df.plot_response(num, den, 0, 20, Ts, alpha_p)
